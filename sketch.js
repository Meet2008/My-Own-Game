var player,invi1,invi2,pet,inviCirc,aliensGroup,bg,bulle,tsGroup,bullets,alien,aliens,safetyLine,fuel,health,bulletsGroup,bulletGroup,bulletLine,inviGroup,li,feGroup,heGroup,left,right,shoot,bbGroup,no,sa,bossGroup;
var f = 500;
var h = 500;
var a = 500;
var count = 0;
...
var count2 = 20;
var score2 = 1;
var shotting,damage,alaram,explosion,playerDamage;
...
function preload(){
	...
	shotting = loadSound("sound/shooting.mp3");
	damage = loadSound("sound/damage.mp3");
	alaram = loadSound("sound/alaram.mp3");
	playerDamage = loadSound("sound/playerDamage.mp3");
}
...
if(score%50 === 0){
	...
	fill("white");
	textSize(24);
	text("boss Health : " + count2,10,120);
}
...
function boss(){
		...
		boss.scale = 0.4;
		boss.debug = true;
	    boss.setCollider("circle",0,0,140);
		bossGroup.add(boss);
}
...
function bossBullets(){
	if(frameCount%60===0){
		...
		bbGroup.add(bulle);
		bulle.debug = true;
		bulle.setCollider("circle",0,0,5);
	} 
}